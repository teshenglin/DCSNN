...
phi  = @(x,y) sqrt((x/ra).^2+(y/rb).^2) - 1;
psi  = @(x,y) hardlims(phi(x,y));
%% Set training and target points
% ... in the interior domain ...
x = bdA+(bdB-bdA)/(2*sqrt(m)):(bdB-bdA)/sqrt(m):bdB-(bdB-bdA)/(2*sqrt(m));
y = bdC+(bdD-bdC)/(2*sqrt(m)):(bdD-bdC)/sqrt(m):bdD-(bdD-bdC)/(2*sqrt(m));
[xx, yy] = meshgrid(x,y);
xin  = [xx(:)'; yy(:)'];
...
% ... on the interface ...
theta = 2*pi*rand(mk,1);
X(1,:) = ra*cos(theta); X(2,:) = rb*sin(theta);
nor(1,:) = (rb*cos(theta))./sqrt( (ra*sin(theta)).^2 + (rb*cos(theta)).^2 );
nor(2,:) = (ra*sin(theta))./sqrt( (ra*sin(theta)).^2 + (rb*cos(theta)).^2 );
% ... jump conditions ...
